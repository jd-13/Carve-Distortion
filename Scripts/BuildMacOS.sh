...
sed -i '' -e 's#../../../../../SDKs/JUCE/modules#JUCE/modules#g' $SCRIPT_DIR/../Carve.jucer

echo "=== Updating include paths ==="
sed -i '' -e 's#../../../WECore/WECore#../../WECore/WECore#g' $SCRIPT_DIR/../Carve.jucer

echo "=== Generating project ==="
$SCRIPT_DIR/../JUCE/Projucer.app/Contents/MacOS/Projucer --resave $SCRIPT_DIR/../Carve.jucer

echo "=== Starting build ==="
cd $SCRIPT_DIR/../Builds/MacOSX
xcodebuild -version
xcodebuild -project Carve.xcodeproj -scheme "Carve - All" -configuration Debug

echo "=== Collecting artefacts ==="
mkdir -p $SCRIPT_DIR/dist
cp -r ~/Library/Audio/Plug-Ins/VST3/Carve.vst3 $SCRIPT_DIR/dist
cp -r ~/Library/Audio/Plug-Ins/Components/Carve.component $SCRIPT_DIR/dist
